...
.content{
  position:relative; z-index:2; max-width:700px; margin:auto; padding:20px;
}
h1{
...
.heart{position:absolute; bottom:0; width:20px; height:20px; background:#ff80c1; opacity:.7; clip-path: polygon(50% 0%,61% 15%,75% 15%,85% 25%,85% 40%,50% 75%,15% 40%,15% 25%,25% 15%,39% 15%); animation:float 5s linear infinite;}
...
.btsLogo{position:absolute; width:30px; height:30px; background:url('bts_logo.png') no-repeat center/contain; opacity:0.8; animation:floatLogo 5s linear forwards;}
...
<div class="content" id="countdownContent">
  ...
  <p class="tagline">Keep shining like BTS and spreading love like ARMY 💜🎶</p>
...
window.onload = ()=>{ musicCountdown.play().catch(()=>{document.getElementById("playMusicCountdown").style.display="inline-block";}); };
document.getElementById("playMusicCountdown").onclick = ()=>{ musicCountdown.play(); document.getElementById("playMusicCountdown").style.display="none"; };
document.getElementById("playMusicBirthday").onclick = ()=>{ musicBirthday.play(); document.getElementById("playMusicBirthday").style.display="none"; };
